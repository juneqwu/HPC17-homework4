...
/* Do the non-blocking send and receive operations */
  for (i=0; i<REPS; i++) {
    ...
    /* The rank 2 processor should never hit the MPI_Waitall, put MPI_Wait after 
       each non-blocking call */
    MPI_Wait(&reqs[offset], &stats[offset]);
    MPI_Irecv(&buf, 1, MPI_INT, src, tag1, COMM, &reqs[offset+1]);
    MPI_Wait(&reqs[offset+1], &stats[offset+1]);
    offset += 2;
    if ((i+1)%DISP == 0)
